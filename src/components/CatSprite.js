...
export default function CatSprite(props) {
  ...
  let activeBlock = workSpaceBlock.map((val) => {
    if (val.isActive) {
      return val.name;
    }
  });

  // console.log(typeof TurnRight15, "ASASSASASSSAS");

  let isRunner = props.data == "WORKING" ? true : false;

  console.log(isRunner, "HAPPY CODING");
...
`;
  ...
  if (isRunner == true) {
    ...
    console.log(activeBlock, "HAHHAHAAHAHAHAHAH");

    console.log(allAnimation, "allllllllllllllll");

    if (props.blockClickType == "whenClick") {
      animationObject = keyframes`
      ${Move10}
      ${TurnLeft15}
      ${TurnRight15}
      `;
    } else if (props.blockClickType == "Move10") {
      animationObject = keyframes`
      ${Move10}
      `;
    } else if (props.blockClickType == "TurnLeft15") {
      animationObject = keyframes`
      ${TurnLeft15}
      `;
    } else if (props.blockClickType == "TurnRight15") {
      animationObject = keyframes`
      ${TurnRight15}
      `;
    } else if (props.blockClickType == "sayHello") {
      reference.current.textContent = "say Hello";
    } else if (props.blockClickType == "sayHello2Sec") {
      reference.current.textContent = "SAY HELLO 2 seconds";

      setTimeout(() => {
        reference.current.textContent = "";
      }, 2000);
    } else if (props.blockClickType == "thinkHmmm") {
      reference.current.textContent = "think Hmmm";
    } else if (props.blockClickType == "thinkHmmm2Sec") {
      reference.current.textContent = "Think Hmmmmmm 2 seconds";
    ...
    RotateRun = styled.div`
      display: inline-block;
      position: relative;
      animation: ${animationObject} 5s linear;
    `;
